...
!   real(8), intent(in) :: lat_edge(nlat+1)			!ESG_SB_20220623+ Changed to intent(inout) to allow for update of variable to avoid crashes for polar domains
   real(8), intent(inout) :: lat_edge(nlat+1)			!ESG_SB_20220623+ Changed to intent(inout) to allow for update of variable to avoid crashes for polar domains
...
data_lat_loop : &
       do j = lat_s,lat_e-1									!ESG_SB_20220623+ Loop over one less latitude to avoid crashes (lat_e-1 instead of lat_e)
         
         if (lat_edge(j+1).LT.10) then								!ESG_SB_20220623+ Update of variable to avoid crashes for polar domains
!	    lat_edge(j+1) = lat_edge(j)+(abs(lat_edge(j)-lat_edge(j-1)))			!ESG_SB_20220623+ Update of variable to avoid crashes for polar domains
	    lat_edge(j) = lat_edge(j-1)								!ESG_SB_20220623+ Update of variable to avoid crashes for polar domains
	    lat_edge(j+1) = lat_edge(j)								!ESG_SB_20220623+ Update of variable to avoid crashes for polar domains
	 endif											!ESG_SB_20220623+ Update of variable to avoid crashes for polar domains
...
data_lon_loop : &
         do dlong_ndx = lon_s,lon_e
           ...
           call llij( real(lat_edge(j),kind=4), real(lon_edge(i),kind=4), proj, x(1), y(1) )	!ESG_SB_
           call llij( real(lat_edge(j),kind=4), real(lon_edge(i+1),kind=4), proj, x(2), y(2) )	!ESG_SB_
           call llij( real(lat_edge(j+1),kind=4), real(lon_edge(i+1),kind=4), proj, x(3), y(3) )	!ESG_SB_
           call llij( real(lat_edge(j+1),kind=4), real(lon_edge(i),kind=4), proj, x(4), y(4) )	!ESG_SB_
