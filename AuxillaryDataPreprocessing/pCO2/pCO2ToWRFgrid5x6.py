...
#Load data
pco2path = '/home/WUR/barte035/WRFChem/AuxillaryDataPreprocessing/oc_v1.6_pCO2_daily.nc'
pco2 = nc.Dataset(pco2path,'r')
print(pco2.variables['pCO2'].shape)
pco2_data = pco2.variables['pCO2'][-1,:,:] #BE SURE TO SELECT THE RIGHT DATE (Data is daily, from 1 January 1982 to 31 December 2017)
...
pco2_data = np.delete(pco2_data,np.s_[0:42],axis=0)

print('aaaaa',pco2_data.shape)
print(np.arange(-180.,180.,5.).shape)
print(np.arange(60.,90.,6.).shape)
...
pco2_trans = m.transform_scalar(pco2_data,lons=np.arange(-180.,180.,6.),lats=np.arange(60.,90.,5.),nx=219,ny=219,order=1)
...
pco2ocean = wrfinput.createVariable('PCO2_5x6_bilinear','f4',('Time','south_north','west_east'))
