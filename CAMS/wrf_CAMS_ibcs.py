...
Further modified by Sjoerd Barten, July 2022 (Updating routine and other subroutines for Anunna and option to load in new CAMSRA (CAMS reanalysis) dataset
...
outpath_macc = './output/'
inpath_wrfinput = '/home/WUR/barte035/WRFChem/WRFV4.1.1/run/'
...
#end_date=dtm.datetime(2020,4,16,0,0)
#21 Days
#start_date=dtm.datetime(2020,4,5,0,0)
#end_date=dtm.datetime(2020,4,22,0,0)
...
def calc_pres_fields(input,tstep,start_date):
    mf=nc.Dataset(input)
    zlen=len(mf.variables['model_level']) #ESG_SB_20250808+ Changed 'level' to 'model_level'
    ...
    file='/home/WUR/barte035/soft/python/lib/ab_ECMWF_L60.in' #path
...
#    xbeg=lonsmacc[0]
    ...
    lm=len(readfile.dimensions['model_level']) #ESG_SB_20250808+ Changed 'level' to 'model_level'
...
#    target=np.zeros((lm,ny,360),dtype=np.float32)    
    target=np.zeros((lm,ny,480),dtype=np.float32)    
    ...
    target[:,:,0:int(nx/2)] = target_copy[:,:,int(nx/2):nx]
    target[:,:,int(nx/2):nx] = target_copy[:,:,0:int(nx/2)]
        
    #target[:,int(js):int(je),int(istart):int(ie)]=outfield[:,int(js):int(je),int(istart):int(ie)]
...
def make_1x1_molefractions(dates=(start_date,end_date),inpath=inpath_macc,outpath=outpath_macc):
    ...
    sys.path.append('/home/WUR/barte035/soft/python/lib')
    
    import tm5tools
#    from ct_netcdf import CT_CDF, std_savedict
    from io4 import CT_CDF, std_savedict
...
#    days = timedate.timegen(sd,ed,dtw)
    days = np.arange(start_date, end_date, dtm.timedelta(days=1)).astype(dtm.datetime)
...
### a proper construction of the 3d_molefraction file. This is to secure start at 0:00 (which is in the file of the previous day)
            ...
            #ncfile = os.path.join(inpath,'CAMS.nc')
            ncfile = os.path.join(inpath,'CAMS_%s.nc'%(new_day.strftime('%Y%m%d')))
	    	
            print('reading MACC input file: CAMS_%s.nc'%(new_day.strftime('%Y%m%d')))
            #print(ncfile)
            ...
            no2 = merge_netcdf_fields(input=ncfile,dsname='no',tstep=tstep,massr=1.,start_date=start_date)
            hcho = merge_netcdf_fields(input=ncfile,dsname='hcho',tstep=tstep,massr=1.,start_date=start_date)
            iso = merge_netcdf_fields(input=ncfile,dsname='c5h8',tstep=tstep,massr=1.,start_date=start_date)
...
#                dimlevs=(ncf.def_dim('levs',nlev),)
                ...
                dimlevs = ncf.createDimension('levs',size=nlev)
                savedict=ncf.StandardVar(varname='levs')
                savedict['name']='levs'
                savedict['long_name']='levs'
                savedict['values']=np.arange(1,nlev+1,1)
...
#                ncf.AddData(savedict)
            ...
            savedict['dims']=dimdate+dimlevs+dimgrid
            savedict['units']='mol mol-1'
            savedict['long_name']='mole_fraction_of_nitrogen_oxide_in_air'
...
## MAQ_lg_20190411+ modified to deal with the MACC data: reduced number of tracers and no NO and NO2
    ...
    specs = ['o3','no','no2','co','hcho','hno3','pan','so2','iso']
    Mspec = [48.e-3,30.01e-3,46.01e-3,28.01e-3,30.03e-3,63.01e-3,121.05e-3,64.07e-3,68.12e-3] #kg/mol
...
### calculating the initial conditions first!
    for each_path in input_paths:
        ...
        init_times=[x.decode('utf-8') for x in init_times[0]]
...
# secondly, vertical interpolation 
            ...
            for iy in range(ny):
                for ix in range(nx): #interp1d requires the pressure values to be increasing, so we reverse the arrays
                    ...
                    Pout_masked = np.array(Pout[valid_Pout].copy())
                    #reverse_Cout = f_vert(Pout[valid_Pout])    
                    reverse_Cout = f_vert(Pout_masked)    
...
def add_bg_to_wrfbdy(inpath_wrf,inpath_macc=outpath_macc,tracer_names=False,okdebug=True):
    ...
    bdy_times2=bdy_times.astype('U13')
    ...
    btimes = [dtm.datetime(int(''.join(bdy_times2[b][:4])),int(''.join(bdy_times2[b][5:7])),int(''.join(bdy_times2[b][8:10])),int(''.join(bdy_times2[b][11:13])),int(''.join(bdy_times2[b][14:16])),int(''.join(bdy_times2[b][17:]))) for b in range(bdy_times2.shape[0])]
...
tracers = ['o3','no','no2','co','hcho','hno3','pan','so2','iso']
