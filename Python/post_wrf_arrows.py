...
cross_start = wrf.CoordPair(lat=51.97, lon=2.7) # start coordinates
cross_end = wrf.CoordPair(lat=51.97, lon=7) # end coordinates

# Select the height for which vertical profile is determined

bot_height = 0. # lowest height
top_height = 5000. # top height
step_size = 100. # height step
...
with PdfPages('plt_crossSection_theta_arrows.pdf') as pdf:
   ...
   var = wrf.getvar(infile,'theta',timeidx=0)
   ...
   lats = [wrf.to_np(x).tolist().lat for x in z_cross.coords['xy_loc']]
   lons = [wrf.to_np(x).tolist().lon for x in z_cross.coords['xy_loc']]
   dlats = np.gradient(lats)
   dlons = np.gradient(lons)
   lat_w = dlats/np.sqrt(dlats**2.+dlons**2.)
   lon_w = dlons/np.sqrt(dlats**2.+dlons**2.)
   
   for coord,coordp1 in zip(zip(lats[:-1],lons[:-1]),zip(lats[1:],lons[1:])): 
        
      ax1.plot([coord[1],coordp1[1]], [coord[0], coordp1[0]],
   ...
   for tracer in ['theta']: 
      ...
      for t  in plot_times: # range(0,len(times),6): #,time_str in enumerate(times):
...
# values and copy these to the missing elements below.
        ...
        uv = wrf.getvar(infile,'uvmet',timeidx=t) # ['u']
        us = uv[0]
        vs = uv[1]
        
        us_cross = wrf.vertcross(us, ht, levels=levels,wrfin=infile,
                    start_point=cross_start,
                    end_point=cross_end,
                    latlon=True, meta=True)
        vs_cross = wrf.vertcross(vs, ht, levels=levels,wrfin=infile,
                    start_point=cross_start,
                    end_point=cross_end,
                    latlon=True, meta=True)
      
        ws = wrf.to_np(us_cross)*lon_w+wrf.to_np(vs_cross)*lat_w
        wa = wrf.getvar(infile,'wa',timeidx=t)
	
        wa_cross = wrf.vertcross(wa, ht, levels=levels,wrfin=infile,
                    start_point=cross_start,
                    end_point=cross_end,
                    latlon=True, meta=True)
        ...
        thinning = 2	
        ax_cross.quiver(xs[::thinning], ys[::thinning],
          ws[::thinning,::thinning], wrf.to_np(wa_cross)[::thinning,::thinning]*100.)
