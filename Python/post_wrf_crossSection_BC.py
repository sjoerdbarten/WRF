...
cross_start = wrf.CoordPair(lat=28.87, lon=83.65) # start coordinates
cross_end = wrf.CoordPair(lat=28.87, lon=83.9) # end coordinates

# Select the height for which vertical profile is determined

bot_height = 2500. # lowest height
top_height = 7700. # top height
step_size = 220. # height step
...
with PdfPages('plt_crossSection_BC.pdf') as pdf:
   ...
   fig.colorbar(cb, orientation='horizontal', pad=0.07)
   
   var = wrf.getvar(infile,'BC',timeidx=0)
...
# values and copy these to the missing elements below.
        ...
        ax_cross.quiver(xs[::thinning], ys[::thinning],
          ws[::thinning,::thinning], wrf.to_np(wa_cross)[::thinning,::thinning]*10.)
