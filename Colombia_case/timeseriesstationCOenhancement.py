...
def timeseriesstation(pollutant,city,figsave):
	...
	echampath = '/archive/ESG/barte035/Colombia/StationData/'
	#echamdatafile = echampath+str(pollutant)+'_level1&2_diurnavg.out'
	#echamdatafile = echampath+str(pollutant)+'_10-34m_January2014.out'
	#echamdatafile = echampath+str(pollutant)+'_10-34m_January2014_WRFtracers.out'
	echamdatafile = echampath+str(pollutant)+'_10-34m_January2014_final2.out'
	echamdatafilepblh = echampath+'PBLH_final.out'
	echamdatafilemet = echampath+'micromet2.out'
	era5datafile = echampath+'ERA5_Bogota_BLH_January2014.txt'
	
	StationData = pd.read_csv(csvfile)
	ncfile = nc.Dataset(wrffile,'r')
	echamdata = pd.read_csv(echamdatafile,delimiter='\s+')
	echamdatapblh = pd.read_csv(echamdatafilepblh,delimiter='\s+',skiprows=2)
	echamdatamet = pd.read_csv(echamdatafilemet,delimiter='\s+')
	era5datapblh = pd.read_csv(era5datafile,delimiter='\s+',skiprows=4,header=None,names=['Timesince','PBLHera5'])
	print(era5datapblh)
	#print(echamdata)
	#print(echamdata.columns)
	#print(echamdatapblh)
	#print(echamdatapblh.columns)
	#print(echamdatamet)
	#print(echamdatamet.columns)
	
	print(ncfile.variables['E_NO'][:10,0,48,34])
	print(ncfile.variables['E_NO'][:10,0,47,34])
	print(ncfile.variables['E_NO'][:10,0,47,35])
	print(ncfile.variables['E_NO'][:10,0,48,35])
	print(ncfile.variables['E_NO'][:10,0,49,35])
	print(ncfile.variables['E_NO'][:10,0,49,34])
	print(ncfile.variables['E_NO'][:10,0,49,33])
	print(ncfile.variables['E_NO'][:10,0,48,33])
	print(ncfile.variables['E_NO'][:10,0,47,33])
	...
	if pollutant != 'NOX':
		if city == 'Cali':
			...
			if plotvar == 'o3':
				plotvar = (ncfile.variables[plotvar][:,0,42,21]*1000.+ncfile.variables[plotvar][:,0,43,21]*1000.+ncfile.variables[plotvar][:,0,43,42]*1000.)/3
	...
	echamdiurnalmean = np.zeros(24)
	echamdiurnalstd = np.zeros(24)
	plotvardiurnalmedian = np.zeros(24)
	echamdiurnalmedian  = np.zeros(24)
	avg2diurnalmedian = np.zeros(24)

	#echamheight = 10.1163
	#echamheight = '9.92190'					#10.1163, 35.2784, 71.6940, 122.028, 188.342, 272.466, 376.328, 501.249, 648.126, 817.831, 1011.10, 1228.59, 1471.27
	echamheight = '9.99175'					#10.1163, 35.2784, 71.6940, 122.028, 188.342, 272.466, 376.328, 501.249, 648.126, 817.831, 1011.10, 1228.59, 1471.27
	#print(echamdata[str(echamheight)].iloc[10:])	
	...
	for i in range(0,24,1):
		...
		plotvardiurnalmedian[i] = np.median(globals()["plotvardiurnal" + str(i)])
		avg2diurnalmedian[i] = np.median(globals()["avg2diurnal" + str(i)])
		#print(echamdata[str(echamheight)].iloc[10+i*2],echamdata[str(echamheight)].iloc[10+i*2+1])
		#echamdiurnalmean[i] = (echamdata[str(echamheight)].iloc[58+i*2]+echamdata[str(echamheight)].iloc[58+i*2+1])/2
		numberofdays = 28
		globals()["echamdiurnal" + str(i)] = np.zeros(numberofdays)
		for j in range(0,numberofdays,1):
			globals()["echamdiurnal" + str(i)][j] = (echamdata[str(echamheight)].iloc[(j*48)+58+i*2]+echamdata[str(echamheight)].iloc[(j*48)+58+i*2+1])/2
		echamdiurnalmean[i] = np.mean(globals()["echamdiurnal" + str(i)])
		echamdiurnalstd[i] = np.std(globals()["echamdiurnal" + str(i)])
		echamdiurnalmedian[i] = np.median(globals()["echamdiurnal" + str(i)])
		#print(echamdiurnalmean[i])
		#print(echamdiurnalstd[i])
	...
	temp = [plotvardiurnalmedian[0],plotvardiurnalmedian[1],plotvardiurnalmedian[2],plotvardiurnalmedian[3],plotvardiurnalmedian[4]]
	plotvardiurnalmedian[0:19] = plotvardiurnalmedian[5:24]
	plotvardiurnalmedian[19:24] = temp
	temp = [avg2diurnalmedian[0],avg2diurnalmedian[1],avg2diurnalmedian[2],avg2diurnalmedian[3],avg2diurnalmedian[4]]
	avg2diurnalmedian[0:19] = avg2diurnalmedian[5:24]
	avg2diurnalmedian[19:24] = temp
	...
	plt.plot(np.arange(24),echamdiurnalmean,color='green',linewidth=2) #ECHAM
	...
	plt.fill_between(np.arange(24),echamdiurnalmean-echamdiurnalstd,echamdiurnalmean+echamdiurnalstd,alpha=0.2,color='green') #ECHAM
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	if pollutant == 'NO':
		plt.ylim(0,120) #for NO, comment for other vars!
	if pollutant == 'O3':
		plt.ylim(0,60)
	...
	#plt.legend(['WRF-Chem','Observations','SCM'],fontsize=7)
	plt.legend(['Observations','SCM'],fontsize=7)
	if figsave == 1:
        	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1D_'+str(city)+''+str(pollutant)+'diurnal_nowrf_new.png',bbox_inches='tight')
	plt.show()
	
	'''
	#for lowest layers
	#echamheight = ['10.1163','35.2784','71.6940','122.028']
	echamheight = ['9.99175','34.8234']
	for h in range(0,len(echamheight),1):
		#print(echamdata[str(echamheight[h])].iloc[10:])	
		...
		for i in range(0,24,1):
			...
			avg2diurnalmean[i] = np.mean(globals()["avg2diurnal" + str(i)])
			avg2diurnalstd[i] = np.std(globals()["avg2diurnal" + str(i)])
			#print(echamdata[str(echamheight[h])].iloc[10+i*2],echamdata[str(echamheight[h])].iloc[10+i*2+1])
			#echamdiurnalmean[i] = (echamdata[str(echamheight)[h]].iloc[58+i*2]+echamdata[str(echamheight[h])].iloc[58+i*2+1])/2
			numberofdays = 28
			globals()["echamdiurnal" + str(i)] = np.zeros(numberofdays)
			for j in range(0,numberofdays,1):
				globals()["echamdiurnal" + str(i)][j] = (echamdata[str(echamheight[h])].iloc[(j*48)+58+i*2]+echamdata[str(echamheight[h])].iloc[(j*48)+58+i*2+1])/2
			echamdiurnalmean[i] = np.mean(globals()["echamdiurnal" + str(i)])
			echamdiurnalstd[i] = np.std(globals()["echamdiurnal" + str(i)])
			#print(echamdiurnalmean[i])
			#print(echamdiurnalstd[i])
			
		#print(echamdiurnalmean)	
			
		if h == 0:
			line0 = np.copy(echamdiurnalmean)
			std0 = np.copy(echamdiurnalstd)
		elif h == 1:
			line1 = np.copy(echamdiurnalmean)
			std1 = np.copy(echamdiurnalstd)
		elif h == 2:
			line2 = np.copy(echamdiurnalmean)
			std2 = np.copy(echamdiurnalstd)
		elif h == 3:
			line3 = np.copy(echamdiurnalmean)
			std3 = np.copy(echamdiurnalstd)
	...
	plt.plot(np.arange(24),plotvardiurnalmean,color='black',linewidth=2)
	plt.plot(np.arange(24),avg2diurnalmean,color='red',linewidth=2)
	plt.plot(np.arange(24),line0,color='green',linewidth=2) #ECHAM
	plt.plot(np.arange(24),line1,color='green',linewidth=2,linestyle='--') #ECHAM
	#plt.plot(np.arange(24),line2,color='green',linewidth=2,linestyle=':') #ECHAM
	#plt.plot(np.arange(24),line3,color='green',linewidth=2,linestyle='-.') #ECHAM
	plt.fill_between(np.arange(24),plotvardiurnalmean-plotvardiurnalstd,plotvardiurnalmean+plotvardiurnalstd,alpha=0.2,color='black')
	plt.fill_between(np.arange(24),avg2diurnalmean-avg2diurnalstd,avg2diurnalmean+avg2diurnalstd,alpha=0.2,color='red')
	plt.fill_between(np.arange(24),line0-std0,line0+std0,alpha=0.2,color='green') #ECHAM
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	if pollutant == 'NO':
	...
	plt.ylabel(''+str(pollegend)+' mixing ratio [ppb]')
	plt.xlabel('Local time [hours]')
	plt.legend(['WRF-Chem','Observations','SCM h=0','SCM h=1','SCM h=2','SCM h=3'],fontsize=7)
	if figsave == 1:
	       	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1D_'+str(city)+''+str(pollutant)+'diurnal_3heights_new.png',bbox_inches='tight')
	plt.show()
	'''
	
	plt.plot(np.arange(24),plotvardiurnalmean,color='black',linewidth=2)
	plt.plot(np.arange(24),avg2diurnalmean,color='red',linewidth=2)
	plt.plot(np.arange(24),echamdiurnalmean,color='green',linewidth=0.5) #ECHAM
	plt.fill_between(np.arange(24),plotvardiurnalmean-plotvardiurnalstd,plotvardiurnalmean+plotvardiurnalstd,alpha=0.2,color='black')
	plt.fill_between(np.arange(24),avg2diurnalmean-avg2diurnalstd,avg2diurnalmean+avg2diurnalstd,alpha=0.2,color='red')
	plt.fill_between(np.arange(24),echamdiurnalmean-echamdiurnalstd,echamdiurnalmean+echamdiurnalstd,alpha=0.1,color='green') #ECHAM
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	if pollutant == 'NO':
		plt.ylim(0,140) #for NO, comment for other vars!
	if pollutant == 'O3':
		plt.ylim(0,50)
	plt.xticks(np.arange(0,23.1,6))
	plt.axvspan(xmin=0.,xmax=6.,ymin=0.97,ymax=1,facecolor='blue',alpha=0.3)
	...
	plt.legend(['WRF-Chem','Observations','SCM'],fontsize=7)
	if figsave == 1:
	       	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1DandWRF_'+str(city)+''+str(pollutant)+'diurnal_mean.png',bbox_inches='tight',dpi=600.)
	plt.show()
	
	#MEDIAN
	plt.plot(np.arange(24),plotvardiurnalmedian,color='black',linewidth=2)
	plt.plot(np.arange(24),avg2diurnalmedian,color='red',linewidth=2)
	plt.plot(np.arange(24),echamdiurnalmedian,color='green',linewidth=0.5) #ECHAM
	plt.fill_between(np.arange(24),plotvardiurnalmedian-plotvardiurnalstd,plotvardiurnalmedian+plotvardiurnalstd,alpha=0.2,color='black')
	plt.fill_between(np.arange(24),avg2diurnalmedian-avg2diurnalstd,avg2diurnalmedian+avg2diurnalstd,alpha=0.2,color='red')
	plt.fill_between(np.arange(24),echamdiurnalmedian-echamdiurnalstd,echamdiurnalmedian+echamdiurnalstd,alpha=0.1,color='green') #ECHAM
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	if pollutant == 'NO':
	...
	plt.legend(['WRF-Chem','Observations','SCM'],fontsize=7)
	if figsave == 1:
	       	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1DandWRF_'+str(city)+''+str(pollutant)+'diurnal_median.png',bbox_inches='tight')
	plt.show()

	
	#PBLH, SWDOWN, albedo STUFF
	pblhwrf = ncfile.variables['PBLH'][:,48,34] #or 49,34 (less stations)
	swdownwrf = ncfile.variables['SWDOWN'][:,48,34] #or 49,34 (less stations)
	albedowrf = ncfile.variables['ALBEDO'][:,48,34] #or 49,34 (less stations)
	plotvardiurnalpblh = pblhwrf[spinup:plotvar.shape[0]]
	plotvardiurnalmeanpblh = np.zeros(24)
	plotvardiurnalstdpblh = np.zeros(24)
	plotvardiurnalswdown = swdownwrf[spinup:plotvar.shape[0]]
	plotvardiurnalmeanswdown = np.zeros(24)
	plotvardiurnalstdswdown = np.zeros(24)
	plotvardiurnalalbedo = pblhwrf[spinup:plotvar.shape[0]]
	plotvardiurnalmeanalbedo = np.zeros(24)
	plotvardiurnalstdalbedo = np.zeros(24)
	echamdiurnalmeanpblh = np.zeros(24)
	echamdiurnalstdpblh = np.zeros(24)
	era5diurnalmeanpblh = np.zeros(8)
	era5diurnalstdpblh = np.zeros(8)

	print(pblhwrf)
	
	numberofdays = 28
	for i in range(0,24,1):
		globals()["plotvardiurnalpblh" + str(i)] = np.zeros(numberofdays) #was 29
		globals()["plotvardiurnalswdown" + str(i)] = np.zeros(numberofdays) #was 29
		globals()["plotvardiurnalalbedo" + str(i)] = np.zeros(numberofdays) #was 29
		for j in range(0,numberofdays,1):	#was 29
			globals()["plotvardiurnalpblh" + str(i)][j] = pblhwrf[(j*24)+i]
			globals()["plotvardiurnalswdown" + str(i)][j] = swdownwrf[(j*24)+i]
			globals()["plotvardiurnalalbedo" + str(i)][j] = albedowrf[(j*24)+i]
		plotvardiurnalmeanpblh[i] = np.mean(globals()["plotvardiurnalpblh" + str(i)])
		plotvardiurnalstdpblh[i] = np.std(globals()["plotvardiurnalpblh" + str(i)])
		plotvardiurnalmeanswdown[i] = np.mean(globals()["plotvardiurnalswdown" + str(i)])
		plotvardiurnalstdswdown[i] = np.std(globals()["plotvardiurnalswdown" + str(i)])
		plotvardiurnalmeanalbedo[i] = np.mean(globals()["plotvardiurnalalbedo" + str(i)])
		plotvardiurnalstdalbedo[i] = np.std(globals()["plotvardiurnalalbedo" + str(i)])
		globals()["echamdiurnal" + str(i)] = np.zeros(numberofdays)
		for j in range(0,numberofdays,1):
			globals()["echamdiurnal" + str(i)][j] = (echamdatapblh[u'EC_PBLHGT'].iloc[(j*48)+58+i*2]+echamdatapblh[u'EC_PBLHGT'].iloc[(j*48)+58+i*2+1])/2
		echamdiurnalmeanpblh[i] = np.mean(globals()["echamdiurnal" + str(i)])
		echamdiurnalstdpblh[i] = np.std(globals()["echamdiurnal" + str(i)])
	
	era5pblhrawdata = era5datapblh['PBLHera5'].values
	
	for i in range(0,8,1):
		globals()["era5diurnal" + str(i)] = np.zeros(numberofdays)
		for j in range(0,numberofdays,1):
			globals()["era5diurnal" + str(i)][j] = (era5pblhrawdata[(j*8)+i])
		era5diurnalmeanpblh[i] = np.mean(globals()["era5diurnal" + str(i)])
		era5diurnalstdpblh[i] = np.std(globals()["era5diurnal" + str(i)])
		
	temppblh = [plotvardiurnalmeanpblh[0],plotvardiurnalmeanpblh[1],plotvardiurnalmeanpblh[2],plotvardiurnalmeanpblh[3],plotvardiurnalmeanpblh[4]]
	plotvardiurnalmeanpblh[0:19] = plotvardiurnalmeanpblh[5:24]
	plotvardiurnalmeanpblh[19:24] = temppblh
	temppblh = [plotvardiurnalstdpblh[0],plotvardiurnalstdpblh[1],plotvardiurnalstdpblh[2],plotvardiurnalstdpblh[3],plotvardiurnalstdpblh[4]]
	plotvardiurnalstdpblh[0:19] = plotvardiurnalstdpblh[5:24]
	plotvardiurnalstdpblh[19:24] = temppblh
	
	tempswdown = [plotvardiurnalmeanswdown[0],plotvardiurnalmeanswdown[1],plotvardiurnalmeanswdown[2],plotvardiurnalmeanswdown[3],plotvardiurnalmeanswdown[4]]
	plotvardiurnalmeanswdown[0:19] = plotvardiurnalmeanswdown[5:24]
	plotvardiurnalmeanswdown[19:24] = tempswdown
	tempswdown = [plotvardiurnalstdswdown[0],plotvardiurnalstdswdown[1],plotvardiurnalstdswdown[2],plotvardiurnalstdswdown[3],plotvardiurnalstdswdown[4]]
	plotvardiurnalstdswdown[0:19] = plotvardiurnalstdswdown[5:24]
	plotvardiurnalstdswdown[19:24] = tempswdown

	tempalbedo = [plotvardiurnalmeanalbedo[0],plotvardiurnalmeanalbedo[1],plotvardiurnalmeanalbedo[2],plotvardiurnalmeanalbedo[3],plotvardiurnalmeanalbedo[4]]
	plotvardiurnalmeanalbedo[0:19] = plotvardiurnalmeanalbedo[5:24]
	plotvardiurnalmeanalbedo[19:24] = tempalbedo
	tempalbedo = [plotvardiurnalstdalbedo[0],plotvardiurnalstdalbedo[1],plotvardiurnalstdalbedo[2],plotvardiurnalstdalbedo[3],plotvardiurnalstdalbedo[4]]
	plotvardiurnalstdalbedo[0:19] = plotvardiurnalstdalbedo[5:24]
	plotvardiurnalstdalbedo[19:24] = tempalbedo
	
	print(era5diurnalmeanpblh)
	0,3,6,9,12,15,18,21
	1,4,7,10,13,16,19,22
	#era5diurnalmeanpblh = np.append(era5diurnalmeanpblh,era5diurnalmeanpblh[0])
	temppblhera5 = [era5diurnalmeanpblh[0],era5diurnalmeanpblh[1]]
	era5diurnalmeanpblh[0:5] = era5diurnalmeanpblh[2:7]
	era5diurnalmeanpblh[6] = temppblhera5[0]
	era5diurnalmeanpblh[7] = temppblhera5[1]
	era5diurnalmeanpblh = np.insert(era5diurnalmeanpblh,0,era5diurnalmeanpblh[-1])
	era5diurnalmeanpblh = np.append(era5diurnalmeanpblh,era5diurnalmeanpblh[1])
	temppblhera5std = [era5diurnalstdpblh[0],era5diurnalstdpblh[1]]
	era5diurnalstdpblh[0:5] = era5diurnalstdpblh[2:7]
	era5diurnalstdpblh[6] = temppblhera5std[0]
	era5diurnalstdpblh[7] = temppblhera5std[1]
	era5diurnalstdpblh = np.insert(era5diurnalstdpblh,0,era5diurnalstdpblh[-1])
	era5diurnalstdpblh = np.append(era5diurnalstdpblh,era5diurnalstdpblh[1])
	
	plt.plot(np.arange(24),plotvardiurnalmeanpblh*1.2,color='black',linewidth=2,zorder=9)
	plt.plot(np.arange(24),echamdiurnalmeanpblh,color='green',linewidth=2,zorder=8) #ECHAM
	#plt.plot([-2,1,4,7,10,13,16,19,22,25],era5diurnalmeanpblh,color='blue',linewidth=2,zorder=7) #ERA5
	plt.fill_between(np.arange(24),plotvardiurnalmeanpblh*1.2-plotvardiurnalstdpblh,plotvardiurnalmeanpblh*1.2+plotvardiurnalstdpblh,alpha=0.2,color='black',zorder=9)
	plt.fill_between(np.arange(24),echamdiurnalmeanpblh-echamdiurnalstdpblh,echamdiurnalmeanpblh+echamdiurnalstdpblh,alpha=0.2,color='green',zorder=8) #ECHAM
	#plt.fill_between([-2,1,4,7,10,13,16,19,22,25],era5diurnalmeanpblh-era5diurnalstdpblh,era5diurnalmeanpblh+era5diurnalstdpblh,alpha=0.2,color='blue',zorder=7) #ERA5
	plt.errorbar([-2,1,4,7,10,13,16,19,22,25], era5diurnalmeanpblh, yerr=era5diurnalstdpblh, fmt='o', color='blue')
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	plt.ylim(0,3000)
	...
	plt.xlabel('Local time [hours]')
	plt.legend(['WRF-Chem','SCM','ERA5'],fontsize=7)
	if figsave == 1:
	       	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1D_'+str(city)+'PBLH_diurnal_new.png',bbox_inches='tight',dpi=600.)
	plt.show()
	
	plt.plot(np.arange(24),plotvardiurnalmeanswdown,color='black',linewidth=2)
	plt.fill_between(np.arange(24),plotvardiurnalmeanswdown-plotvardiurnalstdswdown,plotvardiurnalmeanswdown+plotvardiurnalstdswdown,alpha=0.2,color='black')
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	plt.ylim(0,1000)
	...
	plt.axvspan(xmin=6.,xmax=18.,ymin=0.97,ymax=1,facecolor='yellow',alpha=0.3)
	plt.axvspan(xmin=18.,xmax=24.,ymin=0.97,ymax=1,facecolor='blue',alpha=0.3)
	plt.ylabel('SWDOWN [W m-2]')
	plt.xlabel('Local time [hours]')
	plt.legend(['WRF-Chem'],fontsize=7)
	if figsave == 1:
	       	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1D_'+str(city)+'SWDOWN_new.png',bbox_inches='tight')
	plt.show()

	plt.plot(np.arange(24),plotvardiurnalmeanalbedo,color='black',linewidth=2)
	plt.fill_between(np.arange(24),plotvardiurnalmeanalbedo-plotvardiurnalstdalbedo,plotvardiurnalmeanalbedo+plotvardiurnalstdalbedo,alpha=0.2,color='black')
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	plt.ylim(0,1)
	...
	plt.legend(['WRF-Chem'],fontsize=7)
	if figsave == 1:
	       	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1D_'+str(city)+'ALBEDO_new.png',bbox_inches='tight')
	plt.show()



	#WIND STUFF
	windwrf = (ncfile.variables['U'][:,0,48,34]**2+ncfile.variables['V'][:,0,48,34]**2)**0.5 #or 49,34 (less stations)
	plotvardiurnalwind = windwrf[spinup:plotvar.shape[0]]
	plotvardiurnalmeanwind = np.zeros(24)
	plotvardiurnalstdwind = np.zeros(24)
	echamdiurnalmeanwind = np.zeros(24)
	echamdiurnalstdwind = np.zeros(24)
	ecmwfdiurnalmeanwind = np.zeros(24)
	ecmwfdiurnalstdwind = np.zeros(24)

	print(windwrf)
	
	numberofdays = 28
	for i in range(0,24,1):
		globals()["plotvardiurnalwind" + str(i)] = np.zeros(numberofdays) #was 29
		for j in range(0,numberofdays,1):	#was 29
			globals()["plotvardiurnalwind" + str(i)][j] = windwrf[(j*24)+i]
		plotvardiurnalmeanwind[i] = np.mean(globals()["plotvardiurnalwind" + str(i)])
		plotvardiurnalstdwind[i] = np.std(globals()["plotvardiurnalwind" + str(i)])
		globals()["echamdiurnal" + str(i)] = np.zeros(numberofdays)
		globals()["ecmwfdiurnal" + str(i)] = np.zeros(numberofdays)
		for j in range(0,numberofdays,1):
			globals()["echamdiurnal" + str(i)][j] = (echamdatamet[u'UM+VM'].iloc[(j*48)+58+i*2]+echamdatamet[u'UM+VM'].iloc[(j*48)+58+i*2+1])/2
			globals()["ecmwfdiurnal" + str(i)][j] = (echamdatamet[u'U+V-obs'].iloc[(j*48)+58+i*2]+echamdatamet[u'U+V-obs'].iloc[(j*48)+58+i*2+1])/2
			print(echamdatamet[u'UM+VM'].iloc[(j*48)+58+i*2]-echamdatamet[u'U+V-obs'].iloc[(j*48)+58+i*2])
		echamdiurnalmeanwind[i] = np.mean(globals()["echamdiurnal" + str(i)])
		echamdiurnalstdwind[i] = np.std(globals()["echamdiurnal" + str(i)])
		ecmwfdiurnalmeanwind[i] = np.mean(globals()["ecmwfdiurnal" + str(i)])
		ecmwfdiurnalstdwind[i] = np.std(globals()["ecmwfdiurnal" + str(i)])
		
	tempwind = [plotvardiurnalmeanwind[0],plotvardiurnalmeanwind[1],plotvardiurnalmeanwind[2],plotvardiurnalmeanwind[3],plotvardiurnalmeanwind[4]]
	plotvardiurnalmeanwind[0:19] = plotvardiurnalmeanwind[5:24]
	plotvardiurnalmeanwind[19:24] = tempwind
	tempwind = [plotvardiurnalstdwind[0],plotvardiurnalstdwind[1],plotvardiurnalstdwind[2],plotvardiurnalstdwind[3],plotvardiurnalstdwind[4]]
	plotvardiurnalstdwind[0:19] = plotvardiurnalstdwind[5:24]
	plotvardiurnalstdwind[19:24] = tempwind

	plt.plot(np.arange(24),plotvardiurnalmeanwind,color='black',linewidth=2)
	plt.plot(np.arange(24),echamdiurnalmeanwind,color='green',linewidth=2) #ECHAM
	plt.plot(np.arange(24),ecmwfdiurnalmeanwind,color='blue',linewidth=2) #ECMWF
	plt.fill_between(np.arange(24),plotvardiurnalmeanwind-plotvardiurnalstdwind,plotvardiurnalmeanwind+plotvardiurnalstdwind,alpha=0.2,color='black')
	plt.fill_between(np.arange(24),echamdiurnalmeanwind-echamdiurnalstdwind,echamdiurnalmeanwind+echamdiurnalstdwind,alpha=0.2,color='green') #ECHAM
	plt.fill_between(np.arange(24),ecmwfdiurnalmeanwind-ecmwfdiurnalstdwind,ecmwfdiurnalmeanwind+ecmwfdiurnalstdwind,alpha=0.2,color='blue') #ECHAM
	plt.axvline(x=6,linestyle='--',color='grey')
	plt.axvline(x=18,linestyle='--',color='grey')
	plt.xlim(0,23)
	plt.ylim(0,10)
	...
	plt.xlabel('Local time [hours]')
	plt.legend(['WRF-Chem','SCM','ECMWF'],fontsize=7)
	if figsave == 1:
	       	plt.savefig('/home/WUR/barte035/WRFChem/Python/Colombia/FiguresColombia/1D_'+str(city)+'WIND_diurnal_new.png',bbox_inches='tight')
	plt.show()
