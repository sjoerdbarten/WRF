...
for loop_day in range(1,32):
	for loop_3h_num in range(1,9):
		if loop_3h_num == 1:
			loop_3h = '0-3h'
			e = '/emissions/01/diurnal_cycle/UTC_'+str(loop_3h)+''
			loop_3h_time1 = 0
			loop_3h_time2 = 0
		if loop_3h_num == 2:
			loop_3h = '3-6h'
		...
		print 'domain='+str(domain),'loopday='+str(loop_day),'loop3h='+str(loop_3h)
		...
		for source in range(6):
			...
			for k in range(len(specs)):
				if specs[k]=='CO':
					CO_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_CO[source]/28.01*10e6/(31*24) #mole CO per km2 per hours
				elif specs[k]=='NOx':
					NOx_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_NOx[source]/30.01*10e6/(31*24) #NOx is represented as NO (mole NO per km2 per hours)
				elif specs[k]=='PM25':
					PM25_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_PM25[source]*1000000/(31*24*60*60) #microgr PM25 per m2 per second
				elif specs[k]=='OC':
					OC_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_OC[source]*1000000/(31*24*60*60) #microgr OC per m2 per second
				elif specs[k]=='BC':
					BC_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_BC[source]*1000000/(31*24*60*60) #microgr BC per m2 per second
				elif specs[k]=='SO2':
					SO2_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_SO2[source]/64.02*10e6/(31*24) #mole SO2 per km2 per hours
				elif specs[k]=='C2H6':
					C2H6_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_C2H6[source]/30.07*10e6/(31*24) #mole C2H6 per km2 per hours
				elif specs[k]=='CH3OH':
					CH3OH_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_CH3OH[source]/32.04*10e6/(31*24) #mole CH3OH per km2 per hours
				elif specs[k]=='C2H5OH':
					C2H5OH_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_C2H5OH[source]*46.07/10e6/(31*24) #mole C2H5OH per km2 per hours
				elif specs[k]=='C3H8':
					C3H8_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_C3H8[source]/44.1*10e6/(31*24) #mole C3H8 per km2 per hours
				elif specs[k]=='C2H4':
					C2H4_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_C2H4[source]/28.05*10e6/(31*24) #mole C2H4 per km2 per hours
				elif specs[k]=='C3H6':
					C3H6_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_C3H6[source]/42.08*10e6/(31*24) #mole C3H6 per km2 per hours
				elif specs[k]=='C10H16':
					C10H16_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_C10H16[source]/136.24*10e6/(31*24) #mole C10H16 per km2 per hours
				elif specs[k]=='toluene':
					toluene_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_toluene[source]/92.14*10e6/(31*24) #toluene is C7H8(mole per km2 per hours)
				elif specs[k]=='bigene':
					bigene_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_bigene[source]/56*10e6/(31*24) #mole butene per km2 per hours
				elif specs[k]=='bigalk':
					bigalk_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_bigalk[source]/58*10e6/(31*24) #mole butane per km2 per hours
				elif specs[k]=='CH2O':
					CH2O_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_CH2O[source]/30.03*10e6/(31*24) #CH2O is formaldehyde
				elif specs[k]=='NH3':
					NH3_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_NH3[source]/17.03*10e6/(31*24) #mole NH3 per km2 per hours
				elif specs[k]=='CH3COOH':
					CH3COOH_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_CH3COOH[source]/60.05*10e6/(31*24) #mole CH3COOH per km2 per hours
				else:
					MEK_emissions += DM_emissions* day_frac*diurnal_frac*contribution*EF_MEK[source]/72.11*10e6/(31*24) #mole MEK per km2 per hours
		...
		FINN = Dataset(os.path.join(FINNdatamap,'wrffirechemi_d0'+str(domain)+'_2014-01-'+str(loop_day1)+str(loop_day2)+'_'+str(loop_3h_time1)+str(loop_3h_time2)+':00:00'),'r')
		#we need FINN data to have fire size and faction of land type. These data will be used on plume rise module
		GFED = Dataset('wrffirechemi_d0'+str(domain)+'_2014-01-'+str(loop_day1)+str(loop_day2)+'_'+str(loop_3h_time1)+str(loop_3h_time2)+':00:00','w',format='NETCDF3_CLASSIC')
		#WRF-Chem version 3.2 only allows netcdf version 3 file format with classic type.
	
		print 'wrffirechemi_d0'+str(domain)+'_2014-01-'+str(loop_day1)+str(loop_day2)+'_'+str(loop_3h_time1)+str(loop_3h_time2)+':00:00 has been made'
		...
		Times [:] = ['2','0','1','4','-','0','1','-',''+str(loop_day1)+'',''+str(loop_day2)+'','_',''+str(loop_3h_time1)+'',''+str(loop_3h_time2)+'',':','0','0',':','0','0']
		...
		bigene_emiss = GFED.createVariable('ebu_in_OLI', 'f4',('Time','emissions_zdim_stag', 'south_north', 'west_east'))
		...
		c2h4_emiss = GFED.createVariable('ebu_in_OL2', 'f4', ('Time','emissions_zdim_stag', 'south_north', 'west_east'))
		...
		c2h5oh_emiss = GFED.createVariable('ebu_in_ETH', 'f4', ('Time','emissions_zdim_stag', 'south_north', 'west_east'))
		...
		c3h8_emiss = GFED.createVariable('ebu_in_HC3', 'f4', ('Time','emissions_zdim_stag', 'south_north', 'west_east'))
		...
		c3h6_emiss = GFED.createVariable('ebu_in_OLT', 'f4', ('Time','emissions_zdim_stag', 'south_north', 'west_east'))
		...
		ch2o_emiss = GFED.createVariable('ebu_in_HCHO', 'f4', ('Time','emissions_zdim_stag', 'south_north', 'west_east'))
		...
		toluene_emiss = GFED.createVariable('ebu_in_TOL', 'f4', ('Time','emissions_zdim_stag', 'south_north', 'west_east'))
		...
		ch3cooh_emiss = GFED.createVariable('ebu_in_ORA2', 'f4', ('Time','emissions_zdim_stag', 'south_north', 'west_east'))
