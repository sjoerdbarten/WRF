...
#select dataset
choose_dataset = 11
path = "/home/WUR/barte035/WRFChem/OMI/DATA_AMFrecalc/"
...
vcd_trop_mod = prod.variables['VCD_trop_mod_AV_v2'][0,:,:]
...
vcd_trop_sel_mod = vcd_trop_mod[np.where((errflag == 0))] #parr_rs
vcd_trop_sel_diff = vcd_trop_mod[np.where((errflag == 0))]-vcd_trop[np.where((errflag == 0))]
...
#ALL FILTERS
#amf_trop_sel = amf_trop[np.where((errflag == 0) & (zenithflag < 80) & (amfflag > 0.2) & (cloudflag <= 0.5) & ((snowiceflag < 10) | (snowiceflag == 255)))]
#vcd_trop_sel = vcd_trop[np.where((errflag == 0) & (zenithflag < 80) & (amfflag > 0.2) & (cloudflag <= 0.5) & ((snowiceflag < 10) | (snowiceflag == 255)))]
#clats_sel = clats[np.where((errflag == 0) & (zenithflag < 80) & (amfflag > 0.2) & (cloudflag <= 0.5) & ((snowiceflag < 10) | (snowiceflag == 255)))]
#clons_sel = clons[np.where((errflag == 0) & (zenithflag < 80) & (amfflag > 0.2) & (cloudflag <= 0.5) & ((snowiceflag < 10) | (snowiceflag == 255)))]
...
def plot_singleorbit(parr_rs,clats_rs,clons_rs,r_min,r_max,figtitle,figlabel):
    #parr = orbarray[:,5:26]
    ...
    coll = PolyCollection(pols,array=parr_rs*10E-15,cmap=cm.jet,edgecolors='none',zorder=2,alpha=0.8)
    #difference plots
